...
The measurement and the explanation are different claims, so the table says which it is. Of the 13 entries, 7 are agent behaviour with a cited cause: codex and droid run `exec`, where `/compact` is only a user message (headless, SDK and droid's ACP), and gemini's ACP agent runs tools with `invocation.execute()` directly and fires `SessionStart` only in its non-interactive `main()` (gemini-cli 0.59 source). 3 are about this harness, not the agent: Cursor compaction is backend-initiated and this mock never compacts, and droid's `/compress` needs token accounting the mock model does not provide. 3 begin `observed only` and say what happened without claiming why: droid's prompt hook over ACP, and Cursor's prompt and stop hooks in headless (Cursor runs every hook on a backend request, and this mock sends none).

Revisiting the table in 2026-09 moved three entries off it and changed two causes: claude's SDK compaction fired as soon as the runner actually sent `/compact`; and gemini's ACP tool calls had been failing on this driver's invalid permission reply (`{"outcome":"approved"}` instead of the spec's `{"outcome":{"outcome":"selected","optionId":...}}`), which hid that gemini's ACP path bypasses the tool hooks altogether. Treat `observed only` entries as open questions — twice in one day a confident-sounding cause here was wrong, and both times the real fault was in this harness.
