...
⁶ Cursor's `agent` CLI has no BYOK endpoint: the mock speaks its Connect-protobuf `agent.v1.AgentService` (RunSSE + BidiAppend, schemas read out of the CLI bundle). Server config pins the stream to HTTP/1.1. Headless fires sessionStart and the tool hooks; the TUI also fires beforeSubmitPrompt and stop. ACP (`cursor-agent acp`) fires every hook the backend requests and never sessionStart, and it keeps running after its stdin closes, so the session driver kills it once the exit grace runs out. The CLI sends no built-in tools; the backend chooses them (see "Cursor's tools are chosen by the backend").
...
in ACP, claude/codex/pi in headless. `cursor*` is not measured: the cursor CLI
sends no built-in tools, its backend chooses them, and the column is one
snapshot of what the real backend gave the model in agent mode
(`tests/expected/cursor-tools.json`, cursor-agent 2026.09.23-86fc751,
2026-09-25, the model's own report). "Cursor's tools are chosen by the backend"
below has the per-mode lists and how each tool runs on the client.

| capability | claude | codex | copilot | cursor* | droid | gemini | goose | grok | hermes | kilo | kimi | kiro | omp | opencode | pi | qwen |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| read a file | Read | ↳shell | view | Read | Read | read_file | ↳shell | read_file | read_file | read | Read | read | read | read | read | read_file |
| write a file | Write | ↳shell | ↳shell | Write | Create | write_file | write | write | write_file | write | Write | write | write | write | write | write_file |
| edit in place | Edit | ↳shell | apply_patch | StrReplace | Edit | replace | edit | search_replace | patch | edit | Edit | code | edit | edit | edit | edit |
| run a shell cmd | Bash | exec_command | bash | Shell | Execute | run_shell_command | shell | run_terminal_command | terminal | bash | Bash | shell | bash | bash | bash | run_shell_command |
| find files | ↳shell | ↳shell | glob | Glob | Glob | glob | tree | list_dir | search_files | glob | Glob | glob | glob | glob | ↳shell | glob |
| search contents | ↳shell | ↳shell | rg | Grep | Grep | grep_search | ↳shell | grep | ↳shell | grep | Grep | grep | grep | grep | ↳shell | grep_search |
| todo list | — | — | — | TodoWrite | TodoWrite | — | todo__todo_write | todo_write | todo | todowrite | TodoList | todo_list | todo | todowrite | — | — |
| subagent | Agent | spawn_agent | task | Task | Task | invoke_agent | delegate | spawn_subagent | delegate_task | task | AgentSwarm | subagent | task | task | — | agent |
| skills | Skill | — | skill | — | Skill | activate_skill | load_skill | — | skill_manage | skill | Skill | — | — | skill | — | skill |
| fetch a url | WebFetch | — | web_fetch | WebFetch | — | web_fetch | — | — | — | webfetch | FetchURL | web_fetch | — | webfetch | — | — |
| web search | WebSearch | — | — | WebSearch | — | google_web_search | — | web_search | — | — | — | web_search | web_search | — | — | — |
| plan mode | — | — | — | SwitchMode | ExitSpecMode | enter_plan_mode | — | enter_plan_mode | — | — | EnterPlanMode | — | — | — | — | enter_plan_mode |
| ask the user | — | request_user_input | — | — | — | — | — | ask_user_question | — | question | AskUserQuestion | — | — | — | — | ask_user_question |
| deferred tools | — | — | — | GetDynamicTools | ToolSearch | — | — | search_tool | — | — | — | — | — | — | — | tool_search |
...
**A shell is the only thing every column has.** Every other row has holes. The
next most common is edit-in-place, which only codex lacks, and every agent
...
than a tool call. cursor's snapshot has no skill tool either; its CLI sends
skills in the request context (`agent_skills`).
...
| cursor | — | not swept: the CLI declares no tools, and its tool names are the backend's (see below) |
...
binaries, and cursor declares no tools to cluster on — absence from this
...
#### Cursor's tools are chosen by the backend

The cursor CLI never sends its built-in tools. `supported_tools` (a repeated
`aiserver.v1.ClientSideToolV2`) sits on `StreamUnifiedChatRequest`, which the
CLI does not send; on the `agent.v1` RunSSE + BidiAppend path it does send,
the only tool definitions are MCP ones (`AgentRunRequest.mcp_tools`,
`RequestContext.tools`). The backend picks the built-ins from the mode, the
model and capability fields the client sends. Those fields are the client's
half of the negotiation, and the mock reads them without a key.

`--probe tools` records them as checks (`cursor.flag.<field>`, value as the
answer), along with every `ExecServerMessage` type the installed CLI's bundle
declares (`cursor.exec.<type>`, read from `versions/<build>/index.js`), so a
release that flips a field or adds or drops an exec type is a difference under
`--compare`. The probes run records ACP; a `tools-headless` run records
headless, the mode the snapshot below was taken in. cursor-agent
2026.09.23-86fc751:

| field | ACP | headless |
|-------|-----|----------|
| `run_request.mcp_tools`, `request_context.tools` | 0, 0 | 0, 0 |
| `request_context.supports_mcp_auth` | true | true |
| `request_context.hooks_config` | set | set |
| `request_context.agent_skills`, `custom_subagents` | 0, 0 | 0, 0 |
| `request_context.env.sandbox_enabled` | false | false |
| `request_context.env.sandbox_supported` | unset | false |
| `request_context.env.computer_use_supported` | false | false |
| `request_context.env.secret_redaction_enabled` | unset | unset |
| `request_context.` `web_search_enabled` `web_fetch_enabled` `read_lints_enabled` `search_conversations_enabled` `send_message_enabled` | unset | unset |
| `run_request.` `harness` `client_supports_inline_images` `can_create_cloud_subagents` `suppress_subagent_progress_update_tool` `client_supports_send_to_user` `computer_use_coordinate_mode` `client_supports_prompt_context_usage_rpc` `client_supports_routed_model_update` `system_prompt_spec` `client_supports_preview_card` | unset | unset |

The five `*_enabled` context fields are not set anywhere in the bundle: their
camelCase names do not occur in it, while `supportsMcpAuth:!0` is hard-coded.
The CLI still got `WebSearch`, `WebFetch` and `ReadLints` from the real
backend, so for this client the backend enables them without the flags.

The tools themselves come from one snapshot,
`tests/expected/cursor-tools.json`: the real backend asked through
`cursor-agent -p --trust [--mode plan|ask] --model auto` to list its tool
definitions, 2026-09-25, cursor-agent 2026.09.23-86fc751. It is the model's own
report, not a measurement, and no run updates it. agent mode had 21 tools, plan
adds `CreatePlan` (22), ask drops `SwitchMode` (20). The file also records each
tool's parameters and how it runs on the client:

| tool | exec type the backend sends | evidence |
|------|-----------------------------|----------|
| Shell | `shell_stream_args`, `shell_args`, `background_shell_spawn_args` | schema: `shell_args` and `shell_stream_args` are the same `ShellArgs` (command, working_directory, description, is_background) |
| Read | `read_args` | schema: path, offset, limit |
| Write | `write_args` | schema: path, file_text |
| StrReplace | `write_args` | bundle: no edit exec type; `EditArgs` is path and stream_content only, and the write executor's approval takes a `fileEdit` before/after diff |
| EditNotebook | `write_args` | inferred: no notebook exec type |
| Delete | `delete_args` | schema: path |
| Grep | `grep_args` | schema: every Grep parameter has a `GrepArgs` field |
| Glob | `grep_args` | inferred: no glob exec type; `GlobToolSuccess` carries `GrepFilesResult`'s `client_truncated` and `ripgrep_truncated`; `LsArgs` takes no pattern |
| Task | `subagent_args` | schema: subagent_type, model_id, prompt, resume_agent_id, run_in_background |
| ReadLints | `diagnostics_args` | schema: one path per request |
| CallDynamicTool | `mcp_args` | schema: name, args, provider_identifier, tool_name |
| FetchMcpResource | `read_mcp_resource_exec_args` | schema: server, uri, download_path |
| WebFetch | `fetch_args` | schema: url; `web_fetch_allowlist_precheck_args` is its approval check |
| WebSearch, TodoWrite, CreateGoal, UpdateGoal, SwitchMode, CreatePlan, GetDynamicTools, AwaitShell, GenerateImage | none: run by the backend | bundle: each has an args message in the `ToolCall` oneof (display) and no `ExecServerMessage` type takes it |

`AwaitShell` waits on a shell the backend started with `shell_stream_args`,
whose output it already receives; `subagent_await_args` takes an agent id, not
a shell id. `GetDynamicTools` is answered from the MCP definitions the client
sent. `GenerateImage`'s file may reach disk through `WriteArgs.file_bytes`, the
only exec field that carries binary content; that is not verified.

The run checks that every exec type the snapshot names is still in the
installed CLI (`cursor.snapshot`), and prints the agent-mode list as a
`[tools-snapshot]` line. `tools/capability-matrix.py --snapshot
tests/expected/cursor-tools.json` adds the `cursor*` column.

Of the 41 exec types, the snapshot's tools use 12. The rest are hooks
(`execute_hook_args`), request context, MCP state, approval prechecks, git
diffs, conversation search, subagent control, screen recording and computer
use, and the `pi_*` and `mini_swe_agent_bash_args` types, named after other
agent harnesses (`AgentRunRequest.harness`, unset by this CLI).
...
cursor's tools are not in any of this traffic: the backend chooses them. See
"Cursor's tools are chosen by the backend".
...
`harness-test --harness <agent> --surface <dir>` installs the agent and writes `<agent>.surface.txt` (the top-level commands and long flags its `--help` lists, sorted; `--help-all` where `--help` points at it), `<agent>.version` and the raw `<agent>.help.txt`. Hidden commands are not on it: `cursor-agent acp`, which the registry drives, is not in cursor's `--help` as of 2026.09.23. `--probe hidden` finds those (below). The snapshot holds no version, so a release that changes nothing on the surface stays green. A changed surface fails the `surface (<agent>)` job: that job is the one signal the nightly already has, it names the agent, the diff is in its summary, and the fix is a one-line commit that records when the capability appeared. An issue would be a second channel for the same fact, and one that can be left open. Regenerate:

```bash
IMAGE=tests-test tests/nightly.sh surface <agent> --update
```

#### Hidden commands (`--probe hidden`)

`--probe hidden` asks the CLI instead of its help page. It runs in the default probe run, before the session phase, against the mock, and takes 1 to 34 s per agent (2026-09-25).

- Candidates: a fixed list of protocol and service entry points (`acp`, `mcp`, `mcp-server`, `app-server`, `serve`, `server`, `rpc`, `stdio`, `proto`, `exec`, `worker`, `daemon`, `agent`, `sdk`, `--acp`, `--experimental-acp`, `--stdio`, `--rpc`, `--server`, `--mode rpc`, `--mode acp`, `--output-format acp`), plus every command name registered in the installed package: commander and yargs (`.command("x")`, `new Command("x")`, `command: "x"`), argparse (`add_parser("x")`), click/typer, cobra (`Use: "x"`). The package is found the way `tools/package-sweep.py` finds it: the npm package the binary links into (and its `<name>-<platform>` package), the launcher's own directory, the site-packages package a Python console script imports. JS and Python sources are read, and executables over 1 MB, because bun compiles claude's, kilo's and opencode's JS into the binary as text. Compiled Rust and Go binaries (codex, goose, kiro-cli, grok) have nothing to read; only the fixed list is asked of them.
- A candidate `--help` lists is skipped: the surface snapshot has it.
- `<bin> <candidate> --help`: rejected (unknown command, invalid choice, unexpected argument), the top-level page again (the word was ignored or taken as an argument), or a help page of its own that names the candidate. A flag has no page of its own, so for a flag only a rejection counts.
- Fixed candidates the help did not reject then run bare with an ACP `initialize` line on stdin, all at once, for up to 15 s. A result with `protocolVersion` and `agentCapabilities` is `acp`, any other response to id 1 is `jsonrpc`, other JSON on stdout is `json`. A request at the mock carrying the line's marker or the candidate's words means the agent took it as a prompt; that is logged, not recorded. stdin stays open, so an agent that reads its prompt from stdin waits and never starts the turn. Names read out of the package are never run bare: one of them is `uninstall`.

Each candidate that works and is not listed is `surface/hidden.<name>` (`hidden.flag.<name>` for a flag) with the evidence as the answer: `pass:help`, `pass:acp`, `pass:jsonrpc`, `pass:json`. One that answers `initialize` and is not in the command the registry's driver runs (the ACP command's arguments, `app-server` for codex, `--mode rpc` for pi, `--input-format stream-json` for claude, and the SDK command) is also `surface/hidden.<name>.registry = finding:unused`. That is the check that would have caught cursor: its `acp` answers ACP, is not in its help, and was not in the registry.

Found on 2026-09-25:

| agent | hidden, working | confirmed by |
|---|---|---|
| claude 2.1.282 | `daemon`; `sandbox`, `design-login`, `import-conversations`, `edit-hook`, `edit-chrome-settings`, `edit-memory-settings`, `edit-permission-rules`, `edit-sandbox-settings`, `edit-skill-overrides` | own help page |
| cursor 2026.09.23 | `acp` | ACP `initialize` result (`--help` prints the top-level page) |
| cursor | `local-worker`, `worker-server`, `sandbox`, `repo`, `record`, `install`, `upgrade`, `get-channel`, `set-channel`, `shell-integration`, `cleanup-install-versions`, `repaint-debug` | own help page |
| kimi | `install-app` | own help page |
| opencode | `auth` (prints `providers`' page: an alias), `console` | own help page |
| qwen | `--experimental-acp` | ACP `initialize` result; `finding:unused`, the registry runs `--acp` |

codex, copilot, droid, gemini, goose, grok, hermes, kilo, kiro, omp and pi have none. cursor's `acp` is the only hidden entry point a registry driver uses.
