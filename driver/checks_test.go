...
func TestChecksFailInsteadOfSkip(t *testing.T) {
	...
	cases := []struct {
		...
		{"no requests fails", harness.All["claude"], nil, func(r *Runner) { r.checkAPIRequests("headless", r.entries()) }, 1, 0},
		{"no stream fails", harness.All["claude"], []server.LogEntry{entry(`{"stream":false}`)}, func(r *Runner) { r.checkStreamingFormat("headless", r.entries()) }, 1, 0},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			r := &Runner{harness: c.h, testEntries: c.entries}
			c.run(r)
